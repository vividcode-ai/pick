...
export function ServerSection({ currentUrl, onSave }: ServerSectionProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-[var(--text-primary)]">Server</h3>
        <p className="text-xs text-neutral-500 mt-1">Configure the backend server connection</p>
      </div>
      <div className="settings-card">
        <label className="block text-sm text-[var(--text-secondary)] mb-1.5">Server URL</label>
        <input
          type="text"
          value={url}
          onChange={(e) => setUrl(e.target.value)}
          placeholder="http://localhost:8080"
          className="w-full px-3 py-2 bg-[var(--surface-base)] border border-[var(--border-base)] rounded text-[var(--text-primary)] text-sm outline-none focus:border-blue-500 transition-colors"
        />
