...
export function PermissionDialog({ payload, onRespond }: PermissionDialogProps) {
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    switch (e.key) {
      ...
      case "Tab":
        e.preventDefault();
        setFocusIdx((prev) => (e.shiftKey ? (prev === 0 ? 1 : 0) : prev === 0 ? 1 : 0));
        break;
      case "Enter":
      case " ":
        e.preventDefault();
        focusIdx === 0 ? onRespond(false) : onRespond(true);
        break;
    }
  };
          ...
          <div className="flex items-center justify-between px-4 py-2.5">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors bg-amber-600 text-white hover:bg-amber-500 outline-none ${
