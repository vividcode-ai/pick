...
pub(crate) async fn handle_settings_selection(ctx: &mut TuiContext, val: &str) {
    ...
    match val {
        ...
        other if other.starts_with("theme-") => settings_values::apply_theme(&mut sm, ctx, other).await,
        ...
        other if other.starts_with("ac-max-") => settings_values::apply_ac_max(&mut sm, ctx, other).await,
        ...
        other if other.starts_with("de-") => settings_values::apply_de_action(&mut sm, ctx, other).await,
        other if other.starts_with("tf-") => settings_values::apply_tree_filter(&mut sm, ctx, other).await,
...
fn show_image_width_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let current = sm
        ...
        .unwrap_or(80);
    let items = vec![
        SelectItem::new("60", "image-width-60")
            .with_description(if current == 60 { "current" } else { "" }),
        SelectItem::new("80", "image-width-80")
            .with_description(if current == 80 { "current" } else { "" }),
        SelectItem::new("120", "image-width-120")
            .with_description(if current == 120 { "current" } else { "" }),
    ];
...
fn show_editor_padding_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("0", "editor-padding-0")
            .with_description(if current == 0 { "current" } else { "" }),
        SelectItem::new("1", "editor-padding-1")
            .with_description(if current == 1 { "current" } else { "" }),
        SelectItem::new("2", "editor-padding-2")
            .with_description(if current == 2 { "current" } else { "" }),
        SelectItem::new("3", "editor-padding-3")
            .with_description(if current == 3 { "current" } else { "" }),
    ];
...
fn show_ac_max_visible_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("3", "ac-max-3")
            .with_description(if current == 3 { "current" } else { "" }),
        SelectItem::new("5", "ac-max-5")
            .with_description(if current == 5 { "current" } else { "" }),
        SelectItem::new("7", "ac-max-7")
            .with_description(if current == 7 { "current" } else { "" }),
        SelectItem::new("10", "ac-max-10")
            .with_description(if current == 10 { "current" } else { "" }),
        SelectItem::new("15", "ac-max-15")
            .with_description(if current == 15 { "current" } else { "" }),
        SelectItem::new("20", "ac-max-20")
            .with_description(if current == 20 { "current" } else { "" }),
    ];
...
fn show_steering_mode_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("one-at-a-time", "steering-one-at-a-time")
            .with_description(if current == "one-at-a-time" { "current" } else { "" }),
        SelectItem::new("all", "steering-all")
            .with_description(if current == "all" { "current" } else { "" }),
    ];
...
fn show_follow_up_mode_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("one-at-a-time", "followup-one-at-a-time")
            .with_description(if current == "one-at-a-time" { "current" } else { "" }),
        SelectItem::new("all", "followup-all")
            .with_description(if current == "all" { "current" } else { "" }),
    ];
...
fn show_transport_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("sse", "transport-sse")
            .with_description(if current == "sse" { "current" } else { "" }),
        SelectItem::new("websocket", "transport-websocket")
            .with_description(if current == "websocket" { "current" } else { "" }),
        SelectItem::new("websocket-cached", "transport-websocket-cached")
            .with_description(if current == "websocket-cached" { "current" } else { "" }),
        SelectItem::new("auto", "transport-auto")
            .with_description(if current == "auto" { "current" } else { "" }),
    ];
...
fn show_double_escape_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("tree", "de-tree")
            .with_description(if current == "tree" { "current" } else { "" }),
        SelectItem::new("fork", "de-fork")
            .with_description(if current == "fork" { "current" } else { "" }),
        SelectItem::new("none", "de-none")
            .with_description(if current == "none" { "current" } else { "" }),
    ];
...
fn show_tree_filter_selector(ctx: &mut TuiContext, sm: &SettingsManager) {
    ...
    let items = vec![
        SelectItem::new("default", "tf-default")
            .with_description(if current == "default" { "current" } else { "" }),
        SelectItem::new("no-tools", "tf-no-tools")
            .with_description(if current == "no-tools" { "current" } else { "" }),
        SelectItem::new("user-only", "tf-user-only")
            .with_description(if current == "user-only" { "current" } else { "" }),
        SelectItem::new("labeled-only", "tf-labeled-only")
            .with_description(if current == "labeled-only" { "current" } else { "" }),
        SelectItem::new("all", "tf-all")
            .with_description(if current == "all" { "current" } else { "" }),
    ];
...
fn show_models_selector(
    ...
    let items: Vec<SelectItem> = if models.is_empty() {
        vec![SelectItem::new(model_id.to_string(), model_id.to_string())
