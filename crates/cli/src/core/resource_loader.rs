...
use pick_agent::system_prompt::{self as agent_sp, ContextFile};
...
impl ResourceLoader {
    ...
    pub async fn reload_with_options(
        ...
        if !options.no_context_files {
            self.agents_files = agent_sp::load_project_context_files(&self.cwd, &self.agent_dir);
        }

        // Load system prompt
        self.system_prompt = agent_sp::discover_custom_prompt(&self.agent_dir, &self.cwd);

        // Load append system prompt
        self.append_system_prompt = agent_sp::discover_append_prompt(&self.agent_dir, &self.cwd);
    }
