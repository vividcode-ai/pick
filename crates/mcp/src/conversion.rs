...
pub fn json_schema_from_mcp(input_schema: &serde_json::Map<String, serde_json::Value>) -> JsonSchema {
...
pub fn generate_prompt_snippet(tool_name: &str, input_schema: &serde_json::Map<String, serde_json::Value>) -> String {
    ...
    for item in content {
        match &item.raw {
            ...
            rmcp::model::RawContent::Resource(r) => {
                match &r.resource {
                    ...
                    rmcp::model::ResourceContents::BlobResourceContents { blob, mime_type, .. } => {
                        blocks.push(ContentBlock::image(blob, mime_type.as_deref().unwrap_or("application/octet-stream")));
                    }
                }
            }
            ...
            rmcp::model::RawContent::ResourceLink(r) => {
                blocks.push(ContentBlock::text(&format!("[Resource: {} ({})]", r.uri, r.name)));
            }
        }
    }
