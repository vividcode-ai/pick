...
/// Two-tier settings manager: global (~/.pick/settings.json) + project (.pick/settings.json)
#[derive(Debug)]
...
impl SettingsManager {
    /// Load settings from the given global and project paths
    pub fn load_from_paths(global_path: PathBuf, project_path: PathBuf) -> Self {
    ...
    pub fn reload(&mut self, global_path: &std::path::Path, project_path: &std::path::Path) {
        self.global_settings = load_settings_from_file(global_path);
        self.project_settings = if project_path != self.project_path {
            load_settings_from_file(project_path)
        } else {
            load_settings_from_file(&self.project_path)
        };
        self.project_path = project_path.to_path_buf();
        self.global_path = global_path.to_path_buf();
