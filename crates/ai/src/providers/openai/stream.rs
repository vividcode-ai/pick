    ...
    tokio::spawn(async move {
        ...
        if api_key.is_empty() {
            let mut msg = AssistantMessage::new(
                vec![], "openai-completions".to_string(), "openai".to_string(),
                model.id.clone(), Usage::zero(), StopReason::Error,
            );
            msg.error_message =
                Some(format!("{}_API_KEY is not set.", model.provider.as_str().to_uppercase().replace('-', "_")));
                            ...
                            Ok(chunk) => {
                                ...
                                for sse in events {
                                    process_openai_line(
                                        ...
                                        &mut tool_blocks_by_idx, &mut has_finish_reason,
                                    ).await;
                                }
                            }
                            Err(e) => {
...
async fn process_openai_line(
    ...
    if output.response_id.is_none() {
        output.response_id = chunk.get("id").and_then(|v| v.as_str()).map(|s| s.to_string());
    }
        ...
        for detail in details {
            if let Some(sig) = detail.get("signature").and_then(|v| v.as_str()) {
                ...
                if let Some(ref mut tc) = output.content.get_mut(idx).and_then(|c| {
                    if let ContentBlock::Thinking(tc) = c { Some(tc) } else { None }
                }) {
                    tc.thinking_signature = Some(
                        tc.thinking_signature.clone().unwrap_or_default() + sig
                    );
                }
            }
        }
...
async fn process_tool_call_delta(
    ...
    let stream_idx = tc_val.get("index").and_then(|v| v.as_u64()).map(|v| v as usize);
    ...
    if let Some(pos) = existing_pos {
        ...
        let name = tc_val.get("function")
            .and_then(|f| f.get("name"))
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string();

        let (partial_str, parsed) = tc_val.get("function")
            .map(|f| extract_tool_args(f))
            .unwrap_or_default();

        let content_idx = output.content.len();
        output.content.push(ContentBlock::tool_call(id.clone(), name, parsed.clone()));
...
fn parse_chunk_usage(output: &mut AssistantMessage, usage: &serde_json::Value) {
    if let Some(val) = usage.get("prompt_tokens").or_else(|| usage.get("input_tokens")).and_then(|v| v.as_u64()) {
        output.usage.input = val;
    }
    if let Some(val) = usage.get("completion_tokens").or_else(|| usage.get("output_tokens")).and_then(|v| v.as_u64()) {
