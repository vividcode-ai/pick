...
fn generate_pkce_verifier() -> String {
    ...
    let input = format!("{}-{}-{}", now.as_nanos(), now.as_micros(), std::process::id());
...
const SLOW_DOWN_TIMEOUT_MESSAGE: &str =
...
pub async fn poll_oauth_device_code_flow<F, Fut>(options: OAuthDeviceCodePollOptions<F>) -> Result<String, String>
where
    F: Fn() -> Fut + Send,
    Fut: std::future::Future<Output = OAuthDeviceCodePollResult> + Send,
{
    ...
    let mut interval_ms = std::cmp::max(
        MINIMUM_INTERVAL_MS,
        options.interval_seconds.unwrap_or(DEFAULT_POLL_INTERVAL_SECONDS) * 1000,
    );
