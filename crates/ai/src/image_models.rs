...
static IMAGE_MODEL_REGISTRY: LazyLock<HashMap<String, HashMap<String, ImagesModel>>> =
    LazyLock::new(|| {
        ...
        for (id, name, input_caps, output_caps, cost_input, cost_output, cost_cache_read, cost_cache_write) in [
            ("black-forest-labs/flux.2-flex", "Black Forest Labs: FLUX.2 Flex",
             &["text", "image"] as &[&str], &["image"] as &[&str], 0.0, 0.0, 0.0, 0.0),
            ("black-forest-labs/flux.2-klein-4b", "Black Forest Labs: FLUX.2 Klein 4B",
             &["text", "image"], &["image"], 0.0, 0.0, 0.0, 0.0),
